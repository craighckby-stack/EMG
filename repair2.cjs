...
for (let i = 1; i < blocks.length; i++) {
  ...
  // Re-process anything that was reinstated to see if it missed the truncation flag
  if (block.includes('[HISTORY: struck 2026-09-10 by over-broad heal')) {
    ...
    const isTruncation = lblock.includes('unterminated string literal') || lblock.includes('has no corresponding closing tag') || lblock.includes('expected');
